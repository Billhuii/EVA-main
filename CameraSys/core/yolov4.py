...
# 使用轻量化主干网络（如 cspdarknet53_tiny），仅使用两个特征层（conv_mbbox 和 conv_lbbox）进行目标检测
...
#用于将网络输出的 raw tensor 转换为实际的 bounding box 信息
...
#用于根据分数阈值筛选预测框，并将 xywh 格式转换为 ymin, xmin, ymax, xmax 的形式
...
# YOLO 的三类损失组成：
# GIoU Loss：定位框回归损失。
# Confidence Loss：物体存在与否的置信度损失。
# Class Probability Loss：类别预测误差

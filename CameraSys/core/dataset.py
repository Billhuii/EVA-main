...
class Dataset(object):
    ...
    # 构造函数
    # FLAGS：程序参数（如是否使用tiny模型）
    # is_training：标志是否为训练模式
    # dataset_type：数据集格式，支持converted_coco或yolo
    ...
    #加载标注信息，输出一个字符串列表，每个字符串包含一张图片及其所有bbox信息
    ...
    # 数据集迭代器实现：__iter__, __next__   每次迭代返回一个 batch
    def __iter__(self):
        return self
